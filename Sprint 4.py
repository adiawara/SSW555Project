...
#===================================== US12 Start - Parents not too old ======================================================
#Programmer - Aboubacar Diawara
    for child_id in family['CHIL']:
        if wifeID and indi[wifeID].has_key("BIRT") and indi[child_id].has_key("BIRT"):
            Datedifference5 = (datetime.strptime(indi[child_id]["BIRT"],"%d %b %Y")).year - (datetime.strptime(indi[wifeID]['BIRT'],"%d %b %Y")).year
            if Datedifference5 > 60:
                print "US12 ANOMALY: Fam " + family_id + ": The mother",indi[wifeID]["NAME"]," is more than 60 years older than her child," + indi[child_id]["NAME"],"\n"
        if husbandID and indi[husbandID].has_key("BIRT") and indi[child_id].has_key("BIRT"):
            Datedifference6 = (datetime.strptime(indi[child_id]["BIRT"],"%d %b %Y")).year - (datetime.strptime(indi[wifeID]['BIRT'],"%d %b %Y")).year
            if Datedifference6 > 80:
                print "US12 ANOMALY: Fam " + family_id + ": The father",indi[husbandID]["NAME"]," is more than 60 years older than his child," + indi[child_id]["NAME"],"\n" 
               
#===================================== US12 End - Parents not too old =========================================================
...
#Programmer - Pinkal Gajawala
#Checking for Aniversary date is normal and hence we are not considering it as an anomaly or an error
    if husbandID and wifeID and weddingDate:
        WedDate2 = datetime.strptime(weddingDate,"%d %b %Y").replace(year =today.year)
        if WedDate2 >= today and WedDate2 <= end_date2:
            print "US39 - Family ID:-'",family_id,indi[husbandID]["NAME"],"(Husband) and",indi[wifeID]["NAME"],"(Wife) married on",WedDate,",Their aniversary is within 30 days \n"
